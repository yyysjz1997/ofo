...
int Put_back(int x, int y)
{
	...
	/*打开文件*/
	if ((fpbmp = fopen("bmp\\ditu.bmp", "rb")) == NULL)
	{
		return -1;
	}
	...
	/*行扫描形式读取图片数据并显示*/
	fseek(fpbmp, 54L+(768-y-51)*linebytes, 0);
	for (i = y+49; i > y-1; i--)
	{
		fread(buffer, linebytes, 1, fpbmp);	/*读取一行像素数据*/
...
int Putbmp64k_TMB(int x, int y, const char * path)   //把黑色视为透明的贴图函数
{
	unsigned int temp;
	...
	for (i = height - 1; i > -1; i--)
	{
		...
		for (j = 0; j < width; j++)
		{
			...
			buffer[j].G >>= 2;
			buffer[j].B >>= 3;
			temp=((((unsigned int)buffer[j].R) << 11)
			| (((unsigned int)buffer[j].G) << 5)
			| ((unsigned int)buffer[j].B));
			if (temp>=2)  //让黑的透明
			{
				Putpixel64k(j + x, i + y,temp);	/*计算最终颜色，红绿蓝从高位到低位排列*/
			}
		}
	}
...
int Putbmp64k_TMW(int x, int y, const char * path)   //把白色视为透明的贴图函数
{
	unsigned int temp;
...
int Putbmp64k2(int x, int y, int x1 ,int y1, const char * path)
{
	/*指向图片文件的文件指针*/
	FILE * fpbmp;
